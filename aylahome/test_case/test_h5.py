...
from lib.page.device_h5_page.fresh_air import FreshAir
...
class TestH5():
    ...
    @allure.title('测试设备-{deviceData}')
    @pytest.mark.parametrize("deviceData", DEVICE_LIST_DATA)
    def test_01(self, getDriver, deviceData):
        driver = getDriver
        fip =  FjIndexPage(driver)
        ...
        if isH5 == 1:   #存在H5页面
            try:
                ...
                fa = FreshAir(driver)
                if fa.get_offline_status():
                    with allure.step('切换风速之后并通过接口判断值是否匹配'):
                        fa.switch_windspeed(2)
    ...
    @allure.story("验证罗马电地暖")
    @pytest.mark.skip('不想执行')
    def test_002(self, getDriver):
        driver = getDriver
        fip = FjIndexPage(driver)
        device_name = '罗马电地暖'
        fip.get_element_text(device_name).click()
        sleep(2)
        fip.click_diandinuan()
